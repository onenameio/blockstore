...
use libsigner::v0::messages::{RejectCode, RejectCodeTypePrefix};
...
impl SignerDb {
    ...
    pub fn add_block_rejection_signer_addr(
        ...
        let args = params![
            block_sighash,
            addr.to_string(),
            RejectCodeTypePrefix::from(reject_code) as i64
        ];
    ...
    pub fn get_block_rejection_signer_addrs(
        &self,
        block_sighash: &Sha512Trunc256Sum,
    ) -> Result<Vec<(StacksAddress, RejectCodeTypePrefix)>, DBError> {
        let qry =
        ...
        let rows = stmt.query_map(args, |row| {
            ...
            let reject_code = u8::try_from(reject_code)
                ...
                .and_then(|code| {
                    RejectCodeTypePrefix::from_u8(code).ok_or(SqliteError::InvalidColumnType(
                        1,
                        "reject_code".into(),
                        rusqlite::types::Type::Integer,
                    ))
